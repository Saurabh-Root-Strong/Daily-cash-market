...
PATTERNS = {
    ...
    "spike_up":    "{c} jumps more than {thr:.0f}% in one session",
    "spike_down":  "{c} falls more than {thr:.0f}% in one session",
    "rise_k":      "{c} up more than {thr:.0f}% over {k} sessions",
    "fall_k":      "{c} down more than {thr:.0f}% over {k} sessions",
}
...
STUDY = {
    ...
    "grid": {
        "CRUDE OIL":  dict(tests=1680, nominal=103, chance=84, fdr=0, tradable=0),
        "GOLD":       dict(tests=1507, nominal=39,  chance=75, fdr=0, tradable=0),
        ...
        "COPPER":     dict(tests=1675, nominal=190, chance=84, fdr=4, tradable=1),
    },
    ...
    "survivor_note": {
        "NATURALGAS": (
            ...
            "either way kills it, so it is specific to the next day. It is found "
    ...
    "crude": {
        ...
        # does the last half-year's sign predict the next half-year's? no.
        "regime_sign_agree": 0.43, "regime_halves": 14,
...
def load_commodity(commodity: str, as_of: Optional[date] = None) -> pd.DataFrame:
    ...
    if df.empty:
        return pd.DataFrame(columns=["close", "ret1", "lvl"])
    df["trade_date"] = pd.to_datetime(df.trade_date)
    df = df.set_index("trade_date")
    # the first row of the window has no prior session inside it
...
def get_commodity_state(as_of: date, commodity: str = "CRUDE OIL",
    ...
    if s.mcx_date < as_of:
        s.lag_note = (f"Latest MCX close is {s.mcx_date:%d %b} — MCX trades till "
                      f"23:30, so {as_of:%d %b}'s close lands after the evening sync.")
...
def get_pattern_episodes(as_of: date, commodity: str, index: str, kind: str,
    ...
    for h in ("next1", "next5", "next20"):
        ...
        summary[h] = dict(
            ...
            # ~2 standard errors of a mean of len(e) ordinary sessions: the
            # difference a random sample this size would show half the time... x2
            noise=float(2 * b.std() / np.sqrt(len(e))) if len(e) > 1 else None)
...
def get_rebased_paths(as_of: date, commodity: str, index: str,
    ...
    df = pd.DataFrame({index: ix / ix.iloc[0] * 100,
